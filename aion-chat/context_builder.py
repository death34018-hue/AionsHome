...
TOY_CMD_PATTERN = re.compile(r'\[TOY:([^\]]+)\]')
...
_ALL_CMD_PATTERNS = [
    ...
    HOME_CMD_PATTERN, MCP_CMD_PATTERN, LUCKIN_CMD_PATTERN, TRANSFER_CMD_PATTERN, PRIVATE_WHISPER_CMD_PATTERN,
]
...
async def build_health_summary() -> str:
    ...
    try:
        ...
        async with get_db() as db:
            db.row_factory = aiosqlite.Row
            cur = await db.execute(
                "SELECT steps FROM health_manual_steps WHERE date=?",
                (time.strftime("%Y-%m-%d"),),
            )
            step_row = await cur.fetchone()
            if step_row:
                parts.append(f"今日步数:{step_row['steps']}")

            cur = await db.execute(
                "SELECT sleep_start_time, sleep_end_time, total_min FROM health_sleep_entries "
                "ORDER BY date DESC LIMIT 1"
            )
            sleep_row = await cur.fetchone()
            if sleep_row:
                h = sleep_row["total_min"] // 60
                m = sleep_row["total_min"] % 60
                dur = f"{h}h{m}m" if h else f"{m}m"
                times = ""
                if sleep_row["sleep_start_time"] and sleep_row["sleep_end_time"]:
                    times = f"{sleep_row['sleep_start_time']}-{sleep_row['sleep_end_time']}"
                parts.append(f"睡眠:{dur}" + (f"({times})" if times else ""))
