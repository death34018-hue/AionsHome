...
assert.match(family, /class="role-config" type="button" aria-expanded="false" aria-controls="settings_\$\{role\.actor\}" onclick="toggleRoleSettings\(this\)"/);
assert.match(family, /class="role-config-label">配置<span class="role-config-arrow" aria-hidden="true">/);
assert.match(family, /<div class="settings" id="settings_\$\{role\.actor\}" hidden>/);
assert.doesNotMatch(family, /配置与操作/);
